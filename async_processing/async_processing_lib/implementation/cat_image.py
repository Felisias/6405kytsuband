...
class CatImageGrayscale(CatImageAbstract):
    ...
    def __add__(self, other: 'CatImageGrayscale') -> 'CatImageGrayscale':
        ...
        if self.image.shape != other.image.shape:
            raise ValueError("Cannot add images with different shapes")
        result_img = np.clip(self.image.astype(np.int16) + other.image.astype(np.int16), 0, 255).astype(np.uint8)
        return CatImageGrayscale(result_img, self.url, self.breed)

    def __sub__(self, other: 'CatImageGrayscale') -> 'CatImageGrayscale':
        if not isinstance(other, CatImageGrayscale):
            return NotImplemented
        if self.image.shape != other.image.shape:
            raise ValueError("Cannot subtract images with different shapes")
        result_img = np.clip(self.image.astype(np.int16) - other.image.astype(np.int16), 0, 255).astype(np.uint8)
        return CatImageGrayscale(result_img, self.url, self.breed)
...
class CatImageRGB(CatImageAbstract):
    ...
    def __add__(self, other: 'CatImageRGB') -> 'CatImageRGB':
        ...
        if self.image.shape != other.image.shape:
            raise ValueError("Cannot add images with different shapes")
        result_img = np.clip(self.image.astype(np.int16) + other.image.astype(np.int16), 0, 255).astype(np.uint8)
        return CatImageRGB(result_img, self.url, self.breed)

    def __sub__(self, other: 'CatImageRGB') -> 'CatImageRGB':
        if not isinstance(other, CatImageRGB):
            return NotImplemented
        if self.image.shape != other.image.shape:
            raise ValueError("Cannot subtract images with different shapes")
